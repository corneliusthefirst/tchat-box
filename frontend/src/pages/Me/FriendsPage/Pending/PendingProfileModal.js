...
export default function PendingProfileModal({ user, pending }) {
  ...
  return (
    <div className='flex flex-col lg:w-11/12 xl:w-8/12 md:w-10/12 sm:w-11/12 w-full bg-tchatbox-bgBlackModal'>
      <div
        className={`w-full bg-tchatbox-${
          isIncoming(user, pending) ? pending?.from?.color : pending?.to?.color
              ...
              <a
                ...
                className={`relative flex items-center mx-auto w-20 h-20 bg-tchatbox-${
                  isIncoming(user, pending)
                    ? pending?.from?.color
                    : pending?.to?.color
                } text-white rounded-full inline-block p-2 border-6 border-tchatbox-900`}
              >
                <TchatBox className={`w-12 h-12 text-center mx-auto`} />
              </a>
              <span className='bg-tchatbox-green w-6 h-6 rounded-full absolute right-0 bottom-0 border-6 border-tchatbox-900 mr-0 mb-2'></span>
            </div>
...
function Tabs() {
  ...
  return (
    <div className='w-full flex flex-col mt-16'>
      <ul className='flex w-full justify-start items-start mt-2 py-4 border-b-1 border-tchatbox-600'>
        {tabs.map((tab, i) => (
          <li
            key={i}
